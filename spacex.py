...
def get_scatter_chart(entered_site, slider):
    ...
    if entered_site == 'ALL':
        filtered_df = spacex_df[spacex_df['Payload Mass (kg)'].isin(range(slider[0],slider[1]))]
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color="Booster Version",
        title='launch Site Success Rate')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        filtered_df = filtered_df[filtered_df['Payload Mass (kg)'].isin(range(slider[0],slider[1]))]
